...
typedef struct _it_transform_ {
  ...
  void (*get_output_size) (struct _it_transform_ * transform,
			   idx_t * input_size);

  /* fix the length of the vectors on which to apply the transform */
  void (*set_size) (struct _it_transform_ * transform, idx_t length);
  ...
  void (*get_size) (struct _it_transform_ * transform, idx_t * length);
...
static inline void __it_transform_get_output_size (it_transform_t * t,
						   idx_t * l) {
  t->get_output_size (t, l);
}

#define it_transform_set_size(t, l) __it_transform_set_size(IT_TRANSFORM(t), l)
static inline void __it_transform_set_size (it_transform_t * t, idx_t l) {
  t->set_size (t, l);
}

#define it_transform_get_size(t, l) __it_transform_get_size(IT_TRANSFORM(t), l)
static inline void __it_transform_get_size (it_transform_t * t, idx_t * l) {
  t->get_size (t, l);
}
