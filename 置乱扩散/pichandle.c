...
{  
	...
	printf("Each Block = %d 字节; Block Num = %d 块;\n\n",eachtime,bnum);
	BYTE cpics[picS];//加密后
	memset(cpics,0,picS);
	...
	char epicname[20];
	memset(epicname,0,20);
	cout<<"Please Input The EPic Name..(End with .bmp)"<<endl;
	cin>>epicname;
	FILE *rfp = fopen(epicname,"wb");
	fwrite(&pict.head,1,14,rfp);
	fwrite(&pict.info,1,40,rfp);
	fwrite(cpics,1,picS,rfp);
	fclose(rfp);
	cout<<"已写入密文图片"<<epicname<<endl;
	return 0;  
}
