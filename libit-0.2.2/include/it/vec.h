...
/* remove a warning about unused constants on MacOS X */
  ...
  /*---------------------------------------------------------------------------*//*                Vector types                                               *//*---------------------------------------------------------------------------*//* Notes: the following structures are used to store some vectors values. 
...
/* Never decrease the max size of the vector (call Vec_set_length_max for that purpose) */
...
/* Basic vector functions (specialization of previous #define)                  */
  static inline vec vec_new (idx_t length) {
    return Vec_new (double, length);
  }
  static inline ivec ivec_new (idx_t length) {
    return Vec_new (int, length);
  }
  static inline bvec bvec_new (idx_t length) {
    return Vec_new (byte, length);
  }
  static inline cvec cvec_new (idx_t length) {
    return Vec_new (cplx, length);
  }

  static inline vec vec_new_alloc (idx_t length, idx_t length_max) {
    assert (length_max >= length);
    return Vec_new_alloc (double, length, length_max);
  }
  static inline ivec ivec_new_alloc (idx_t length, idx_t length_max) {
    assert (length_max >= length);
    return Vec_new_alloc (int, length, length_max);
  }
  static inline bvec bvec_new_alloc (idx_t length, idx_t length_max) {
    assert (length_max >= length);
    return Vec_new_alloc (byte, length, length_max);
  }
  static inline cvec cvec_new_alloc (idx_t length, idx_t length_max) {
    assert (length_max >= length);
    return Vec_new_alloc (cplx, length, length_max);
  }

  static inline void vec_delete (vec v) {
    Vec_delete (v);
  }
  static inline void ivec_delete (ivec v) {
    Vec_delete (v);
  }
  static inline void bvec_delete (bvec v) {
    Vec_delete (v);
  }
  static inline void cvec_delete (cvec v) {
    Vec_delete (v);
  }

  static inline idx_t vec_length (vec v) {
    return Vec_length (v);
  }
  static inline idx_t ivec_length (ivec v) {
    return Vec_length (v);
  }
  static inline idx_t bvec_length (bvec v) {
    return Vec_length (v);
  }
  static inline idx_t cvec_length (cvec v) {
    return Vec_length (v);
  }

  static inline idx_t vec_length_max (vec v) {
    return Vec_length_max (v);
  }
  static inline idx_t ivec_length_max (ivec v) {
    return Vec_length_max (v);
  }
  static inline idx_t bvec_length_max (bvec v) {
    return Vec_length_max (v);
  }
  static inline idx_t cvec_length_max (cvec v) {
    return Vec_length_max (v);
  }

  static inline vec _vec_set_length (vec v, idx_t N) {
    Vec_set_length (v, N);
    return v;
  }
  static inline ivec _ivec_set_length (ivec v, idx_t N) {
    Vec_set_length (v, N);
    return v;
  }
  static inline bvec _bvec_set_length (bvec v, idx_t N) {
    Vec_set_length (v, N);
    return v;
  }
  static inline cvec _cvec_set_length (cvec v, idx_t N) {
    Vec_set_length (v, N);
    return v;
  }
...
#define cvec_set_length( v, N ) do { v=_cvec_set_length( v, N ); } while (0)
  ...
  static inline vec _vec_set_length_max (vec v, idx_t N) {
    Vec_set_length_max (v, N);
    return v;
  }
  static inline ivec _ivec_set_length_max (ivec v, idx_t N) {
    Vec_set_length_max (v, N);
    return v;
  }
  static inline bvec _bvec_set_length_max (bvec v, idx_t N) {
    Vec_set_length_max (v, N);
    return v;
  }
  static inline cvec _cvec_set_length_max (cvec v, idx_t N) {
    Vec_set_length_max (v, N);
    return v;
  }
...
#define cvec_set_length_max( v, N ) do { v=_cvec_set_length_max( v, N ); } while (0)
  ...
  static inline vec vec_init (vec v, double *buf, idx_t N) {
    return (vec) Vec_init (v, buf, N);
  }
  static inline ivec ivec_init (ivec v, int *buf, idx_t N) {
    return (ivec) Vec_init (v, buf, N);
  }
  static inline bvec bvec_init (bvec v, byte * buf, idx_t N) {
    return (bvec) Vec_init (v, buf, N);
  }
  static inline cvec cvec_init (cvec v, cplx * buf, idx_t N) {
...
#define _cvec(v, i) (*__cvec(v, i))
  ...
  static inline vec vec_set (vec v, double val) {
    Vec_set (v, val);
    return v;
  }
  static inline ivec ivec_set (ivec v, int val) {
    Vec_set (v, val);
    return v;
  }
  static inline bvec bvec_set (bvec v, byte val) {
    Vec_set (v, val);
    return v;
  }
  static inline cvec cvec_set (cvec v, cplx val) {
    Vec_set (v, val);
    return v;
  }

  static inline vec vec_set_between (vec v, idx_t i1, idx_t i2, double val) {
    Vec_set_between (v, i1, i2, val);
    return v;
  }
  static inline ivec ivec_set_between (ivec v, idx_t i1, idx_t i2, int val) {
    Vec_set_between (v, i1, i2, val);
    return v;
  }
  static inline bvec bvec_set_between (bvec v, idx_t i1, idx_t i2, byte val) {
    Vec_set_between (v, i1, i2, val);
    return v;
  }
  static inline cvec cvec_set_between (cvec v, idx_t i1, idx_t i2, cplx val) {
    Vec_set_between (v, i1, i2, val);
    return v;
  }

  static inline vec vec_get_subvector (vec v, idx_t i1, idx_t i2) {
    vec  s;
    VEC_END_PARAM (v, i1);
    VEC_END_PARAM (v, i2);
    s = vec_new (i2 - i1 + 1);
  ...
  static inline ivec ivec_get_subvector (ivec v, idx_t i1, idx_t i2) {
    ivec s;
    VEC_END_PARAM (v, i1);
    VEC_END_PARAM (v, i2);
    s = ivec_new (i2 - i1 + 1);
  ...
  static inline bvec bvec_get_subvector (bvec v, idx_t i1, idx_t i2) {
    bvec s;
    VEC_END_PARAM (v, i1);
    VEC_END_PARAM (v, i2);
    s = bvec_new (i2 - i1 + 1);
  ...
  static inline cvec cvec_get_subvector (cvec v, idx_t i1, idx_t i2) {
    cvec s;
    VEC_END_PARAM (v, i1);
    VEC_END_PARAM (v, i2);
    s = cvec_new (i2 - i1 + 1);
  ...
  static inline void vec_set_subvector (vec v, vec s, idx_t idx) {
    VEC_END_PARAM (v, idx);
    Vec_set_subvector (v, s, idx);
  }
  static inline void ivec_set_subvector (ivec v, ivec s, idx_t idx) {
    VEC_END_PARAM (v, idx);
    Vec_set_subvector (v, s, idx);
  }
  static inline void bvec_set_subvector (bvec v, bvec s, idx_t idx) {
    VEC_END_PARAM (v, idx);
    Vec_set_subvector (v, s, idx);
  }
  static inline void cvec_set_subvector (cvec v, cvec s, idx_t idx) {
    VEC_END_PARAM (v, idx);
    Vec_set_subvector (v, s, idx);
  }
...
/* Copy the content of a vector into another vector                            
  ...
  static inline void vec_copy (vec dest, vec orig) {
    Vec_copy (dest, orig);
  }
  static inline void ivec_copy (ivec dest, ivec orig) {
    Vec_copy (dest, orig);
  }
  static inline void bvec_copy (bvec dest, bvec orig) {
    Vec_copy (dest, orig);
  }
  static inline void cvec_copy (cvec dest, cvec orig) {
    Vec_copy (dest, orig);
  }

/* The same function as copy, but which allocate some memory for the destination vector */
  static inline Vec Vec_clone (Vec v) {
    assert (v);
  ...
  static inline ivec ivec_clone (ivec v) {
    assert (v);
    return ((ivec) Vec_copy (ivec_new (ivec_length (v)), v));
  }
  static inline bvec bvec_clone (bvec v) {
    assert (v);
    return ((bvec) Vec_copy (bvec_new (bvec_length (v)), v));
  }
  static inline cvec cvec_clone (cvec v) {
    assert (v);
...
/* Common functions                                                             */
  ...
  void cvec_abssqr (cvec v);	/* elementwise |v_i|^2 */
...
/* Useful operations                                                            */
  ...
  double vec_sum_robust(vec v); /* Robust version */ 
...
/* The following function are based on the merge sorting algorithm. The values are returned 
   in the increasing and decreasing orders for the min and the max, respectively */
  ...
  double vec_variance_robust (vec v); /* Robust version */
...
/* Set operations !!! Some of these operations are currently not implemented    */
  static inline vec vec_del (vec v, idx_t pos) {
    Vec_del (v, pos);
    return v;
  }
  static inline ivec ivec_del (ivec v, idx_t pos) {
    Vec_del (v, pos);
    return v;
  }
  static inline bvec bvec_del (bvec v, idx_t pos) {
    Vec_del (v, pos);
    return v;
  }
  static inline cvec cvec_del (cvec v, idx_t pos) {
    Vec_del (v, pos);
    return v;
  }

/* To be used as v = ivec_ins( v, pos, elt ) */
  static inline vec _vec_ins (vec v, idx_t pos, double elt) {
    Vec_ins (v, pos, elt);
    return v;
  }
  static inline ivec _ivec_ins (ivec v, idx_t pos, int elt) {
    Vec_ins (v, pos, elt);
    return v;
  }
  static inline bvec _bvec_ins (bvec v, idx_t pos, byte elt) {
    Vec_ins (v, pos, elt);
    return v;
  }
  static inline cvec _cvec_ins (cvec v, idx_t pos, cplx elt) {
    Vec_ins (v, pos, elt);
    return v;
  }
...
/* Stack operations: Vector may be used as a stack                              */
  static inline vec _vec_push (vec v, double elt) {
    vec_ins (v, vec_length (v), elt);
    return v;
  }
  static inline ivec _ivec_push (ivec v, int elt) {
    ivec_ins (v, ivec_length (v), elt);
    return v;
  }
  static inline bvec _bvec_push (bvec v, byte elt) {
    bvec_ins (v, bvec_length (v), elt);
    return v;
  }
  static inline cvec _cvec_push (cvec v, cplx elt) {
...
#define cvec_push( v, elt ) do { v=_cvec_push( v, elt ); } while (0)
  ...
  static inline vec vec_pop (vec v) {
    Vec_pop (v);
    return v;
  }
  static inline ivec ivec_pop (ivec v) {
    Vec_pop (v);
    return v;
  }
  static inline bvec bvec_pop (bvec v) {
    Vec_pop (v);
    return v;
  }
  static inline cvec cvec_pop (cvec v) {
    Vec_pop (v);
    return v;
  }

  static inline double vec_head (vec v) {
    return Vec_head (v);
  }
  static inline int ivec_head (ivec v) {
    return Vec_head (v);
  }
  static inline byte bvec_head (bvec v) {
    return Vec_head (v);
  }
  static inline cplx cvec_head (const cvec v) {
    return Vec_head (v);
  }
...
/* Vectors of zeros */
  static inline vec vec_new_zeros (idx_t N) {
    return (vec_new_set (0., N));
  }
  static inline ivec ivec_new_zeros (idx_t N) {
    return (ivec_new_set (0, N));
  }
  static inline bvec bvec_new_zeros (idx_t N) {
    return (bvec_new_set (0, N));
  }
  static inline cvec cvec_new_zeros (idx_t N) {
    return (cvec_new_set (cplx_0, N));
  }

/* Vectors of ones */
  static inline vec vec_new_ones (idx_t N) {
    return (vec_new_set (1., N));
  }
  static inline ivec ivec_new_ones (idx_t N) {
    return (ivec_new_set (1, N));
  }
  static inline bvec bvec_new_ones (idx_t N) {
    return (bvec_new_set (1, N));
  }
  static inline cvec cvec_new_ones (idx_t N) {
