...
/*
  Matrices functions
  Copyright (C) 2005 Vivien Chappelier, Herve Jegou, Francois Cayre
*/
...
#define Mat_new_alloc( type_t, h, w, hmax, wmax ) \
  ...
  static inline Mat __Mat_new_alloc (size_t elem_size, idx_t h, idx_t w,
...
/* double matrices functions                                       */
  ...
  static inline mat mat_new (idx_t h, idx_t w) {
    return (Mat_new (double, h, w));
  }
  static inline imat imat_new (idx_t h, idx_t w) {
    return (Mat_new (int, h, w));
  }
  static inline bmat bmat_new (idx_t h, idx_t w) {
    return (Mat_new (byte, h, w));
  }
  static inline cmat cmat_new (idx_t h, idx_t w) {
    return (Mat_new (cplx, h, w));
  }

  static inline mat mat_new_alloc (idx_t h, idx_t w, idx_t hmax, idx_t wmax) {
    return (Mat_new_alloc (double, h, w, hmax, wmax));
  }
  static inline imat imat_new_alloc (idx_t h, idx_t w, idx_t hmax, idx_t wmax) {
    return (Mat_new_alloc (int, h, w, hmax, wmax));
  }
  static inline bmat bmat_new_alloc (idx_t h, idx_t w, idx_t hmax, idx_t wmax) {
    return (Mat_new_alloc (byte, h, w, hmax, wmax));
  }
  static inline cmat cmat_new_alloc (idx_t h, idx_t w, idx_t hmax, idx_t wmax) {
    return (Mat_new_alloc (cplx, h, w, hmax, wmax));
  }

  static inline void mat_delete (mat m) {
    Mat_delete (m);
  }
  static inline void imat_delete (imat m) {
    Mat_delete (m);
  }
  static inline void bmat_delete (bmat m) {
    Mat_delete (m);
  }
  static inline void cmat_delete (cmat m) {
    Mat_delete (m);
  }

  static inline idx_t mat_height (mat m) {
    return Mat_height (m);
  }
  static inline idx_t mat_height_max (mat m) {
    return Mat_height_max (m);
  }
  static inline idx_t mat_width (mat m) {
    return Mat_width (m);
  }
  static inline idx_t mat_width_max (mat m) {
    return Mat_width_max (m);
  }

  static inline idx_t imat_height (imat m) {
    return Mat_height (m);
  }
  static inline idx_t imat_height_max (imat m) {
    return Mat_height_max (m);
  }
  static inline idx_t imat_width (imat m) {
    return Mat_width (m);
  }
  static inline idx_t imat_width_max (imat m) {
    return Mat_width_max (m);
  }

  static inline idx_t bmat_height (bmat m) {
    return Mat_height (m);
  }
  static inline idx_t bmat_height_max (bmat m) {
    return Mat_height_max (m);
  }
  static inline idx_t bmat_width (bmat m) {
    return Mat_width (m);
  }
  static inline idx_t bmat_width_max (bmat m) {
    return Mat_width_max (m);
  }

  static inline idx_t cmat_height (cmat m) {
    return Mat_height (m);
  }
  static inline idx_t cmat_height_max (cmat m) {
    return Mat_height_max (m);
  }
  static inline idx_t cmat_width (cmat m) {
    return Mat_width (m);
  }
  static inline idx_t cmat_width_max (cmat m) {
    return Mat_width_max (m);
  }


  static inline mat _mat_set_height (mat m, idx_t h) {
    Mat_set_height (m, h);
    return m;
  }
  static inline mat _mat_set_height_max (mat m, idx_t hmax) {
...
#define mat_set_height_max( m, h ) do { m = _mat_set_height_max(m, h); } while(0)
  ...
  static inline imat _imat_set_height (imat m, idx_t h) {
    Mat_set_height (m, h);
    return m;
  }
  static inline imat _imat_set_height_max (imat m, idx_t hmax) {
    Mat_set_height_max (m, hmax);
    return m;
  }
...
#define imat_set_height_max( m, h ) do { m = _imat_set_height_max(m, h); } while(0)
  ...
  static inline bmat _bmat_set_height (bmat m, idx_t h) {
    Mat_set_height (m, h);
    return m;
  }
  static inline bmat _bmat_set_height_max (bmat m, idx_t hmax) {
    Mat_set_height_max (m, hmax);
    return (m);
  }
...
#define bmat_set_height_max( m, h ) do { m = _bmat_set_height_max(m, h); } while(0)
  ...
  static inline cmat _cmat_set_height (cmat m, idx_t h) {
    Mat_set_height (m, h);
    return m;
  }
  static inline cmat _cmat_set_height_max (cmat m, idx_t hmax) {
    Mat_set_height_max (m, hmax);
    return (m);
  }
...
#define cmat_set_height_max( m, h ) do { m = _cmat_set_height_max(m, h); } while(0)
  ...
  static inline mat _mat_init (mat m, double *buf, idx_t w, idx_t h) {
    Mat_init (m, buf, w, h);
    return (m);
  }
  static inline imat _imat_init (imat m, int *buf, idx_t w, idx_t h) {
    Mat_init (m, buf, w, h);
    return (m);
  }
  static inline bmat _bmat_init (bmat m, byte * buf, idx_t w, idx_t h) {
    Mat_init (m, buf, w, h);
    return (m);
  }
  static inline cmat _cmat_init (cmat m, cplx * buf, idx_t w, idx_t h) {
    Mat_init (m, buf, w, h);
    return (m);
  }
...
#define cmat_init(m, buf, w, h) _cmat_init(m, buf, w, h)
  ...
  static inline mat mat_set (mat m, double val) {
    Mat_set (m, val);
    return m;
  }
  static inline imat imat_set (imat m, int val) {
    Mat_set (m, val);
    return m;
  }
  static inline bmat bmat_set (bmat m, byte val) {
    Mat_set (m, val);
    return m;
  }
  static inline cmat cmat_set (cmat m, cplx val) {
    Mat_set (m, val);
    return m;
  }

  static inline mat mat_set_between (mat m, idx_t r1, idx_t c1, idx_t r2,
				     idx_t c2, double val) {
    Mat_set_between (m, r1, c1, r2, c2, val);
    return m;
  }
  static inline imat imat_set_between (imat m, idx_t r1, idx_t c1, idx_t r2,
				       idx_t c2, int val) {
    Mat_set_between (m, r1, c1, r2, c2, val);
    return m;
  }
  static inline bmat bmat_set_between (bmat m, idx_t r1, idx_t c1, idx_t r2,
				       idx_t c2, byte val) {
    Mat_set_between (m, r1, c1, r2, c2, val);
    return m;
  }
  static inline cmat cmat_set_between (cmat m, idx_t r1, idx_t c1, idx_t r2,
				       idx_t c2, cplx val) {
    Mat_set_between (m, r1, c1, r2, c2, val);
    return m;
  }
...
/* Remove a row from a matrix */
  static inline mat __mat_del_row (mat m, idx_t pos) {
    Mat_del_row (m, pos);
    return m;
  }
  static inline imat __imat_del_row (imat m, idx_t pos) {
    Mat_del_row (m, pos);
    return m;
  }
  static inline bmat __bmat_del_row (bmat m, idx_t pos) {
    Mat_del_row (m, pos);
    return m;
  }
  static inline cmat __cmat_del_row (cmat m, idx_t pos) {
    Mat_del_row (m, pos);
    return m;
  }
...
/* Insert a row */
  ...
  static inline mat __mat_ins_row (mat m, idx_t pos, vec v) {
    Mat_ins_row (m, pos, v);
    return m;
  }
  static inline imat __imat_ins_row (imat m, idx_t pos, ivec v) {
    Mat_ins_row (m, pos, v);
    return m;
  }
  static inline bmat __bmat_ins_row (bmat m, idx_t pos, bvec v) {
    Mat_ins_row (m, pos, v);
    return m;
  }
  static inline cmat __cmat_ins_row (cmat m, idx_t pos, cvec v) {
    Mat_ins_row (m, pos, v);
    return m;
  }
...
/* Add a vector at the end row of the matrix (Use matrix as a vector stack)  */
  static inline mat __mat_push_row (mat m, vec v) {
    mat_ins_row (m, mat_height (m), v);
    return m;
  }
  static inline imat __imat_push_row (imat m, ivec v) {
    imat_ins_row (m, imat_height (m), v);
    return m;
  }
  static inline bmat __bmat_push_row (bmat m, bvec v) {
    bmat_ins_row (m, bmat_height (m), v);
    return m;
  }
  static inline cmat __cmat_push_row (cmat m, cvec v) {
    cmat_ins_row (m, cmat_height (m), v);
    return m;
  }

#define mat_push_row( m, pos )  do { m = __mat_push_row( m, pos ); } while (0)
#define imat_push_row( m, pos ) do { m = __imat_push_row( m, pos ); } while (0)
#define bmat_push_row( m, pos ) do { m = __bmat_push_row( m, pos ); } while (0)
#define cmat_push_row( m, pos ) do { m = __cmat_push_row( m, pos ); } while (0)


/* Retrieve the last element at the end of the vector (Use vector as a stack)*/
  static inline mat __mat_pop_row (mat m) {
    Mat_pop_row (m);
    return m;
  }
  static inline imat __imat_pop_row (imat m) {
    Mat_pop_row (m);
    return m;
  }
  static inline bmat __bmat_pop_row (bmat m) {
    Mat_pop_row (m);
    return m;
  }
  static inline cmat __cmat_pop_row (cmat m) {
    Mat_pop_row (m);
    return m;
  }
...
/* Retrieve part or assemble matrices                               */
  ...
  static inline void mat_copy_row (vec v, mat m, idx_t r) {
    assert (m);
  ...
  static inline void imat_copy_row (ivec v, imat m, idx_t r) {
    assert (m);
    MAT_END_ROW_PARAM (m, r);
  ...
  static inline void bmat_copy_row (bvec v, bmat m, idx_t r) {
    assert (m);
    MAT_END_ROW_PARAM (m, r);
  ...
  static inline void cmat_copy_row (cvec v, cmat m, idx_t r) {
    assert (m);
    MAT_END_ROW_PARAM (m, r);
  ...
  static inline vec mat_get_row (mat m, idx_t r) {
    vec  v;
    assert (m);
  ...
  static inline ivec imat_get_row (imat m, idx_t r) {
    ivec v;
    assert (m);
  ...
  static inline bvec bmat_get_row (bmat m, idx_t r) {
    bvec v;
    assert (m);
  ...
  static inline cvec cmat_get_row (cmat m, idx_t r) {
    cvec v;
    assert (m);
  ...
  static inline vec mat_get_col (mat m, idx_t c) {
    vec  v;
    assert (m);
  ...
  static inline ivec imat_get_col (imat m, idx_t c) {
    ivec v;
    assert (m);
  ...
  static inline bvec bmat_get_col (bmat m, idx_t c) {
    bvec v;
    assert (m);
  ...
  static inline cvec cmat_get_col (cmat m, idx_t c) {
    cvec v;
    assert (m);
...
/*------------------------------------------------------------------*/
  ...
  static inline mat mat_new_zeros (idx_t h, idx_t w) {
    return (mat_new_set (0., h, w));
  }
  static inline imat imat_new_zeros (idx_t h, idx_t w) {
    return (imat_new_set (0, h, w));
  }
  static inline bmat bmat_new_zeros (idx_t h, idx_t w) {
    return (bmat_new_set (0, h, w));
  }
  static inline cmat cmat_new_zeros (idx_t h, idx_t w) {
    return (cmat_new_set (cplx_0, h, w));
  }

  static inline mat mat_new_ones (idx_t h, idx_t w) {
    return (mat_new_set (1., h, w));
  }
  static inline imat imat_new_ones (idx_t h, idx_t w) {
    return (imat_new_set (1, h, w));
  }
  static inline bmat bmat_new_ones (idx_t h, idx_t w) {
    return (bmat_new_set (1, h, w));
  }
  static inline cmat cmat_new_ones (idx_t h, idx_t w) {
